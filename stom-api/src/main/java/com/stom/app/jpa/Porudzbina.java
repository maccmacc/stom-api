...
public class Porudzbina implements Serializable {
	...
	public String getLookup(){
		return id+", "+dobavljac.getNaziv()+", "+ datum;
	}
	
	public BigDecimal getUkupno() {
		double u = 0;
		if (stavkaPorudzbines != null) {
			Iterator<StavkaPorudzbine> it = stavkaPorudzbines.iterator();
			while (it.hasNext()) {
				StavkaPorudzbine sp = it.next();
				u = u + new BigDecimal(sp.getCena().doubleValue() * sp.getKolicina().doubleValue()).doubleValue(); 
			}
		}
